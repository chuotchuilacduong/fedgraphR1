...
#### 5. Run Federated GRPO Experiment (Multi-process Simulation)
```bash
# Run 3 simulated FL clients across iterations
nohup bash -u run_fed_grpo.sh -p Qwen/Qwen2.5-3B-Instruct -m Qwen2.5-3B-Instruct -d 2WikiMultiHopQA -c 3 -r 40 > result_fed_grpo.log 2>&1 &
```

#### 6. Close retrieve server 8001 port
```bash
fuser -k 8001/tcp
```
...
### Federated Training & Graph Checkpointing
When running Graph-R1 in federated mode (via `script_federated_build.py`), the server automatically checkpoints its state into the directory specified by `checkpoint_dir` in `fl_config.py`.

- **Global LoRA parameters** (`lora_round_{N}.npz`) are saved **every round**.
- **Global Hypergraph Contexts** (`server_hypergraph_round_{N}.graphml`) are saved every `checkpoint_freq` rounds.

#### Resume Procedure
If a federated training run crashes or a client drops out, the system implements a native resume behavior:
1. Reload the target `lora_round_{N}.npz` directly onto your `FederatedRayPPOTrainer` (client-side) or pass it into the Server's aggregation mechanism via `start_flower_server`.
2. Reload `server_hypergraph_round_{N}.graphml` into `NetworkXStorage`.
3. Start the node agents. **Dropout/Resume Resilience is built-in**: When a client rejoins the cluster, their delta-cache will automatically be invalidated. During the first "re-joined" training round, the client will forcefully re-upload their full Hypergraph contexts instead of delta slices, seamlessly bringing the entire FL Server back into full synchrony.

### Privacy Limitations
Please note that in this prototype, entity descriptions extracted during Round 0 and subsequent training rounds contain raw semantic content. Full Differential Privacy (DP) injections acting on parameter exchanges and content hashing are explicitly deferred to future releases. Do not run this framework currently over sensitive production databases (such as direct patient PI) without introducing your own DP layer.
